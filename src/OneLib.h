/* -*- C++ -*- */
  ...
  namespace OneLib {
    ...
    template<>inline void OnePinBase::mode<OUTPUT>() {modeOut();}
    template<>inline void OnePinBase::mode<INPUT>() {modeIn();}
    template<>inline void OnePinBase::mode<INPUT_PULLUP>() {modeInUp();}

    template<class O>
    class OnePin:public OnePinBase  {
      public:
        // VPin():pin(O()) {}
        OnePin(O& o):pin(o) {}
